...
class TaskCard(View):
    def get(self,request,task_slug,action):
        pass

    def post(self,request,task_slug,action):
        post_dict={
            'task_movement':self.task_movement,
            'addsubtask':self.addsubtask,
            'toggle_subtask':self.toggle_subtask,
        }
        return post_dict[action](request,task_slug)
    
    def task_movement(self, request, task_slug):
        try:
            broadcast_data = self.update_task(request, task_slug)
            if broadcast_data:
    ...
    def update_task(self,request,task_slug):
        ...
        if new_status=='PROGRESS':
            ...
            for reviewer in Assign.objects.filter(task=task,role='REVIEWER'):
                ActivityLog.objects.update_or_create(
                    ...
                    task=task,
                    workspace=workspace,
                    defaults={'type': 'REVIEW_REQUESTED'}
                )
            ...
            ActivityLog.objects.update_or_create(
                ...
                task=task,
                workspace=workspace,
                defaults={'type': 'ISSUE_CREATED '}
            )
            for rev in Assign.objects.filter(task=task,role='REVIEWER'):
                Review.objects.update_or_create(
                    task=task,
                    reviewer=rev.assigned_to,
                    defaults={'status': 'ISSUE'}
                )
                ActivityLog.objects.update_or_create(
    ...
    def addsubtask(self,request,task_slug):
        try:
            data=json.loads(request.body) if request.body else {}
    ...
    def toggle_subtask(self,request,task_slug):
        try:
